...
class Member extends CI_Model{
  ...
  public function insert($data1 = array(), $data2 = array()) {
	...
	public function getall($id){
		//var_dump($id);
		//exit;
	$ret=$this->db->select('id,cat_id,cat_name,saleteam_id,conti_asign_id')
	->where('saleteam_id',$id)
 ->get('zoho_category_conti_user');
  //$query = $this->db->get('');
 ...
 $this->db->from('zoho_category_conti_user');
  ...
  return $query->result_array();  
	...
	$catname=$this->db->select('cat_id,cat_name,saleteam_id,conti_asign_id,sales_per_name')
